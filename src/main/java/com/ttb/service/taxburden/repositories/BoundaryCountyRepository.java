...
public interface BoundaryCountyRepository extends CrudRepository<BoundaryCountyEntity, Long> {
    ...
    @Query(value = "select pde from #{#entityName} bce, PoliticalDivisionEntity pde " +
            "where st_contains(bce.geom, :location) = true and bce.statefp || bce.countyfp = pde.fips")
    ...
    @Query(value = "select pde from #{#entityName} bce, BoundaryPostalCodeEntity bpce, PoliticalDivisionEntity pde " +
            "where bpce.zcta5ce10 = :postalCode and st_intersects(bce.geom, bpce.geom) = true and bce.statefp || bce.countyfp = pde.fips")
