...
public interface BoundaryPlaceRepository extends CrudRepository<BoundaryPlaceEntity, Long> {
    ...
    @Query(value = "select bpe.geom from #{#entityName} bpe, BoundaryPostalCodeEntity bpce, PoliticalDivisionEntity pde " +
            "where bpce.zcta5ce10 = :postalCode and st_intersects(bpe.geom, bpce.geom) = true and bpe.statefp || bpe.placefp = pde.fips")
    List<Geometry> placesCovered(String postalCode);

    @Query(value = "select bpce.geom from BoundaryPostalCodeEntity bpce where bpce.zcta5ce10 = :postalCode")
    Geometry postalBoundary(String postalCode);
