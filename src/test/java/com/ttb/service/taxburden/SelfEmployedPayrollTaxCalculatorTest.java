...
import com.ttb.service.taxburden.calculation.SelfEmployedPayrollTaxCalculator;
...
public class SelfEmployedPayrollTaxCalculatorTest {
	...
	private SelfEmployedPayrollTaxCalculator selfEmployedPayrollTaxCalculator;
	...
	public void setup() {
		...
		TaxRateEntity taxRateOne = new TaxRateEntity(TEST_PAYROLL_TAX_ONE, BigDecimal.valueOf(0.124), BigInteger.valueOf(0), BigInteger.valueOf(127200));
		...
		TaxRateEntity taxRateOneSINGLE = new TaxRateEntity(TEST_PAYROLL_TAX_TWO, BigDecimal.valueOf(0.124), BigInteger.valueOf(0), BigInteger.valueOf(127200));
	...
	public void calculateDefaultTaxFilingStatusTest() {
		selfEmployedPayrollTaxCalculator = new SelfEmployedPayrollTaxCalculator();
		selfEmployedPayrollTaxCalculator.setTaxRateSetRepository(mockTaxRateSetRepository);
		selfEmployedPayrollTaxCalculator.init();
		...
		TaxDefinitionEntity taxDefinition = new TaxDefinitionEntity(TaxType.INCOME_STATE, politicalDivisionGA.getFips(), "selfEmployedPayrollTaxCalculator", TEST_PAYROLL_TAX_ONE, "test description", 1);
		...
		TaxPayerProfileEntity taxPayerProfile = new TaxPayerProfileEntity(politicalDivisions, new MonetaryAmountEntity(BigDecimal.valueOf(100000)), new MonetaryAmountEntity(BigDecimal.valueOf(50000)), new MonetaryAmountEntity(BigDecimal.valueOf(0)), new MonetaryAmountEntity(BigDecimal.valueOf(0)),
				new MonetaryAmountEntity(BigDecimal.valueOf(200000)), TaxPayerProfileEntity.BASIC_CONSUMER_EXPENDITURE_PROFILE_KEY, TaxFilingStatus.JOINT, new MonetaryAmountEntity(BigDecimal.valueOf(15000)), new MonetaryAmountEntity(BigDecimal.valueOf(5000)), Integer.valueOf(3));
		MonetaryAmountEntity result = null;
		try {
			result = selfEmployedPayrollTaxCalculator.calculate(taxPayerProfile, politicalDivisionGA, taxDefinition, taxBurdenReport);
	...
	public void calculateJointTaxFilingStatusTest() {
		...
		politicalDivisions.add(politicalDivisionUS);
		politicalDivisions.add(politicalDivisionGA);
		TaxDefinitionEntity taxDefinition = new TaxDefinitionEntity(TaxType.INCOME_STATE, politicalDivisionGA.getFips(), "selfEmployedPayrollTaxCalculator", TEST_PAYROLL_TAX_TWO, "test description", 1);
		...
		assertNotNull(result);
		assertEquals(4250, result.getAmount().longValue());
	}
}
